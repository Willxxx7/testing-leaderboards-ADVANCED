...
def run_comprehensive_tests():
    ...
    Handles Render.com cold starts and slow loading
    """
    ...
    print("⚠️  Note: If using Render.com free tier, the first load may take 30-60 seconds")
    ...
    show_browser = input("\n👁️ Show browser during testing? (recommended for debugging): (y/n): ").strip().lower()
    ...
    with sync_playwright() as p:
        ...
        # Test 1: Page loads successfully (with retry for Render)
        print("\n📋 Test 1: Page Load Test (with cold start handling)")
        start_time = time.time()
        
        # Increase timeout for Render.com sites
        timeout_ms = 90000  # 90 seconds for cold start
        max_retries = 2
        ...
        for attempt in range(max_retries + 1):
            try:
                if attempt > 0:
                    print(f"  🔄 Retry {attempt}/{max_retries} (waiting for Render to wake up)...")
                    time.sleep(10)
        ...
        if load_success and response and response.status == 200:
            status = "passed"
            print(f"  ✅ Page loaded in {load_time:.0f}ms (Status: {response.status})")
            print(f"  💡 Cold start took {load_time/1000:.1f} seconds")
        else:
            status = "failed"
            print(f"  ❌ Failed to load after {load_time/1000:.1f} seconds")
            print("  💡 Tip: Make sure your Render service is deployed and URL is correct")
            
        test_results.append({
            "title": "Page loads successfully",
            "status": status,
            "duration": load_time,
            "details": f"Status code: {response.status if response else 'N/A'}, Load time: {load_time/1000:.1f}s"
        })
        
        if status == "failed":
            print("\n❌ Cannot continue testing - page failed to load")
            print("💡 Check: Is your Render service deployed? Try opening the URL in a browser first.")
            browser.close()
            return None
        
        # Test 2: Wait for dynamic content to load (important for Render)
        print("\n📋 Test 2: Dynamic Content Loading")
        start_time = time.time()
        
        # Wait for any content to appear
        try:
            # Wait for body to have content (not just loading spinner)
            ...
            content_load_time = (time.time() - start_time) * 1000
            print(f"  ✅ Dynamic content loaded in {content_load_time:.0f}ms")
            test_results.append({
                "title": "Dynamic content loads",
                "status": "passed",
                "duration": content_load_time,
                "details": f"Content loaded in {content_load_time/1000:.1f}s"
            })
        except:
            ...
            test_results.append({
                ...
                "details": "Content load time exceeded 30 seconds"
            })
        
        # Test 3: Title verification
        print("\n📋 Test 3: Page Title Check")
        start_time = time.time()
        ...
        # Test 4: Main heading exists
        print("\n📋 Test 4: Main Heading Check")
        start_time = time.time()
        try:
        ...
        # Test 5: Look for game/score related content
        print("\n📋 Test 5: Game/Score Content Detection")
        start_time = time.time()
        ...
        # Test 6: Numbers/Statistics detection (scores, players, etc.)
        print("\n📋 Test 6: Statistics Display Check")
        start_time = time.time()
        ...
        try:
            ...
            if 'player' in page_content:
                stat_patterns.append("players")
            ...
            # Try to find specific stats visible in the page
            game_players = page.locator('text=/\\d+ Game Players/')
            test_scores = page.locator('text=/\\d+ Test Scores/')
            ...
            if has_stats:
                ...
                if game_players.count() > 0:
                    print(f"     📊 Game Players: {game_players.text_content()}")
                if test_scores.count() > 0:
                    print(f"     📊 Test Scores: {test_scores.text_content()}")
                if best_time.count() > 0:
                    print(f"     📊 Best Time: {best_time.text_content()}")
            else:
        ...
        # Test 7: Empty state messages (helpful for debugging)
        print("\n📋 Test 7: Empty State Detection")
        start_time = time.time()
        try:
            empty_patterns = [
                'no scores yet', 'no data', 'no results', 'empty', 
                'be the first', 'nothing yet', 'no entries', '0 players', '0 tests'
            ]
            ...
            for pattern in empty_patterns:
                ...
                if elements.count() > 0:
                    found_empty = True
                    empty_text = elements.first.text_content()
                    print(f"     ℹ️ Found: '{empty_text[:50]}'")
                    break
            
            test_results.append({
                "title": "Empty state handling",
                "status": "passed",  # Not a failure if no empty state
                "duration": (time.time() - start_time) * 1000,
                "details": "Empty state messages present" if found_empty else "No empty state messages"
            })
            
            if found_empty:
                print(f"  ℹ️ Empty state detected (site has no data yet - this is normal for new leaderboards)")
            else:
                print(f"  ✅ No empty state messages (site may have data)")
        except Exception as e:
            test_results.append({
                "title": "Empty state handling",
                "status": "passed",
                "duration": (time.time() - start_time) * 1000,
                "details": "Could not check empty states"
            })
            print(f"  ⚠️ Could not check empty states")
        ...
        page.reload()
        time.sleep(2)  # Wait for reload and potential errors
        ...
        try:
            ...
            if load_time_ms < 3000:
                ...
                perf_status = "warning"
                perf_msg = f"Okay load time: {load_time_ms}ms (Render cold start may cause this)"
            else:
                perf_status = "warning"  # Changed from failed to warning for Render
                perf_msg = f"Slow load time: {load_time_ms}ms - typical for Render free tier cold start"
            ...
            print(f"  { '✅' if perf_status == 'passed' else '⚠️'} {perf_msg}")
        except:
            test_results.append({
                "title": "Good performance",
                "status": "warning",
        ...
        # Test 12: Leaderboard structure
        print("\n📋 Test 12: Leaderboard Structure Check")
        start_time = time.time()
        ...
        if 'onrender.com' in url:
            print(f"\n💡 Render.com Note:")
            print(f"   • Cold starts typically take 30-60 seconds for free tier")
            print(f"   • Your page loaded in {load_time/1000:.1f} seconds")
            print(f"   • Consider keeping your service active with a cron job or uptime monitor")
...
<body>
    <div class="container">
        ...
        <div class="footer">
            ...
            <p>⚠️ For Render.com free tier, first load may take 30-60 seconds (cold start)</p>
        </div>
    </div>
</body>
</html>"""
